# Portable path resolution (works as .ps1 and compiled .exe)
if ($PSScriptRoot) {
    $Global:GraphAuditXRoot = $PSScriptRoot
} else {
    $Global:GraphAuditXRoot = Split-Path -Parent ([System.Diagnostics.Process]::GetCurrentProcess().MainModule.FileName)
}
Write-Host "RUNNING FILE:" $MyInvocation.MyCommand.Path
Write-Host "APP ROOT:" $Global:GraphAuditXRoot

. (Join-Path $Global:GraphAuditXRoot "M365Api\M365-ManagementAPI.ps1")
...
# LOAD DATA
# ================================
Import-Module (Join-Path $Global:GraphAuditXRoot "GraphAuditX\GraphAuditX.psd1") -Force
$configPath = Join-Path $Global:GraphAuditXRoot "AuditOperationsGateConfig.csv"
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    ...
    <Grid Margin="20">
        ...
        <StackPanel Grid.Row="5" Margin="0,10,0,10">
            ...
            <TextBlock Text="🔍 HAR File Analysis" FontWeight="Bold" Foreground="#00BFFF" FontSize="14"/>

            <StackPanel Orientation="Horizontal" Margin="0,5,0,5">
                <Button Name="HarBrowseBtn"
                        Content="📂 Browse HAR File"
                        Width="150"
                        Margin="0,0,10,0"/>
                <TextBlock Name="HarFileLabel" Text="No file loaded" Foreground="#888" VerticalAlignment="Center" FontSize="11"/>
            </StackPanel>

            <TabControl Name="HarTabControl" Height="220" Background="#2D2D2D" Foreground="#FFF" Visibility="Collapsed">
                <TabItem Header="Search Criteria" Foreground="#000">
                    <DataGrid Name="HarSearchGrid"
                ...
                <TabItem Header="All Requests" Foreground="#000">
                    <DataGrid Name="HarRequestsGrid"
                              AutoGenerateColumns="True"
                              IsReadOnly="True"
                              Background="#1E1E1E"
                              Foreground="#E0E0E0"
                ...
                <TabItem Header="Summary" Foreground="#000">
                    <TextBox Name="HarSummaryBox"
                             IsReadOnly="True"
                             FontFamily="Consolas"
                             FontSize="11"
                             Background="#1E1E1E"
...
$HarBrowseBtn  = $window.FindName("HarBrowseBtn")
$HarFileLabel  = $window.FindName("HarFileLabel")
$HarTabControl = $window.FindName("HarTabControl")
$HarSearchGrid = $window.FindName("HarSearchGrid")
$HarRequestsGrid = $window.FindName("HarRequestsGrid")
$HarSummaryBox = $window.FindName("HarSummaryBox")
...
# HAR BROWSE BUTTON
# ================================
$HarBrowseBtn.Add_Click({
    $dialog = New-Object System.Windows.Forms.OpenFileDialog
    $dialog.Filter = "HAR Files (*.har)|*.har|All Files (*.*)|*.*"
    $dialog.Title = "Select HAR File (Browser Network Trace)"

    if ($dialog.ShowDialog() -eq "OK") {
        try {
            $HarFileLabel.Text = "Analyzing: $(Split-Path $dialog.FileName -Leaf)..."
            $StatusBox.Text = "Parsing HAR file..."

            $harContent = Get-Content $dialog.FileName -Raw -Encoding UTF8
            $har = $harContent | ConvertFrom-Json

            # Find all Purview/Compliance audit entries
            $auditEntries = @()
            $allRequests = @()
            foreach ($entry in $har.log.entries) {
                ...
                if ($url -match "purview|compliance|auditLog|AuditLogSearch|adtsch") {
                    $allRequests += [PSCustomObject]@{
                        Timestamp  = $ts
                        Method     = $method
                        Status     = $status
                        URL        = if ($url.Length -gt 120) { $url.Substring(0,120) + "..." } else { $url }
                        FullURL    = $url
                    }
                }

                if ($url -match "AuditLogSearch\?pageSize" -or $url -match "auditLog/queries") {
                    $responseBody = $null
                    ...
                    # Extract search criteria from response pageResult
                    if ($responseBody -and $responseBody.pageResult) {
                        foreach ($page in $responseBody.pageResult) {
                            $req = $null
                            if ($page.request) {
                                try {
                                    $reqStr = if ($page.request -is [string]) { $page.request } else { $page.request | ConvertTo-Json -Depth 1 }
                                    $req = $reqStr | ConvertFrom-Json
                                } catch {}
                            }
                            if ($req) {
                                $auditEntries += [PSCustomObject]@{
                                    SearchName   = $page.name
                                    StartDate    = if ($req.StartDateUtc) { $req.StartDateUtc } else { $req.filterStartDateTime }
                                    EndDate      = if ($req.EndDateUtc) { $req.EndDateUtc } else { $req.filterEndDateTime }
                                    RecordTypes  = if ($req.RecordTypes) { ($req.RecordTypes -join ", ") } elseif ($req.recordTypeFilters) { ($req.recordTypeFilters -join ", ") } else { "" }
                                    Operations   = if ($req.Operations) { ($req.Operations -join ", ") } elseif ($req.operationFilters) { ($req.operationFilters -join ", ") } else { "" }
                                    Users        = if ($req.Users) { ($req.Users -join ", ") } elseif ($req.userPrincipalNameFilters) { ($req.userPrincipalNameFilters -join ", ") } else { "" }
                                    FreeText     = if ($req.FreeText) { $req.FreeText } else { "" }
                                    IPAddresses  = if ($req.IPAddresses) { ($req.IPAddresses -join ", ") } else { "" }
                                    Status       = $page.status
                                    CreatedBy    = if ($page.createdBy) { $page.createdBy } else { "" }
                                }
                            }
                        }
                    }

                    # Also check direct response fields (Graph API style)
                    if ($responseBody -and (-not $responseBody.pageResult) -and ($responseBody.filterStartDateTime -or $responseBody.FilterStartDateTime)) {
                        $r = $responseBody
                        $auditEntries += [PSCustomObject]@{
                            SearchName   = if ($r.displayName) { $r.displayName } elseif ($r.Name) { $r.Name } else { "Query" }
                            StartDate    = if ($r.filterStartDateTime) { $r.filterStartDateTime } else { $r.FilterStartDateTime }
                            EndDate      = if ($r.filterEndDateTime) { $r.filterEndDateTime } else { $r.FilterEndDateTime }
                            RecordTypes  = if ($r.recordTypeFilters) { ($r.recordTypeFilters -join ", ") } elseif ($r.RecordTypeFilters) { ($r.RecordTypeFilters -join ", ") } else { "" }
                            Operations   = if ($r.operationFilters) { ($r.operationFilters -join ", ") } elseif ($r.OperationFilters) { ($r.OperationFilters -join ", ") } else { "" }
                            Users        = if ($r.userPrincipalNameFilters) { ($r.userPrincipalNameFilters -join ", ") } elseif ($r.UserPrincipalNameFilters) { ($r.UserPrincipalNameFilters -join ", ") } else { "" }
                            FreeText     = ""
                            IPAddresses  = ""
                            Status       = if ($r.status) { $r.status } else { $r.Status }
                            CreatedBy    = if ($r.createdBy) { $r.createdBy } else { "" }
                        }
                    }
                }
            }
            ...
            # Build summary
            $summary = "================================================================`r`n"
            $summary += "  HAR FILE ANALYSIS SUMMARY`r`n"
            $summary += "================================================================`r`n"
            $summary += "File: $(Split-Path $dialog.FileName -Leaf)`r`n"
            $summary += "Total HTTP entries: $($har.log.entries.Count)`r`n"
            $summary += "Purview/Audit requests: $($allRequests.Count)`r`n"
            $summary += "Search jobs found: $($auditEntries.Count)`r`n`r`n"

            if ($auditEntries.Count -gt 0) {
                $summary += "--- SEARCH CRITERIA EXTRACTED ---`r`n"
                $idx = 0
                foreach ($e in $auditEntries) {
                    $idx++
                    $summary += "`r`nSearch #$idx : $($e.SearchName)`r`n"
                    $summary += "  Date Range : $($e.StartDate)  ->  $($e.EndDate)`r`n"
                    if ($e.RecordTypes) { $summary += "  Record Types: $($e.RecordTypes)`r`n" }
                    if ($e.Operations) { $summary += "  Operations  : $($e.Operations)`r`n" }
                    if ($e.Users) { $summary += "  Users       : $($e.Users)`r`n" }
                    if ($e.FreeText) { $summary += "  Free Text   : $($e.FreeText)`r`n" }
                    if ($e.IPAddresses) { $summary += "  IP Addresses: $($e.IPAddresses)`r`n" }
                    $summary += "  Status      : $($e.Status)`r`n"
                }
            }

            # Check for AuditEnabled/Disabled indicators
            $disabledHits = ($har.log.entries | Where-Object {
                $_.response.content.text -match "AuditingDisabledTenant|AuditEnabled.*false"
            }).Count
            if ($disabledHits -gt 0) {
                $summary += "`r`n*** WARNING: Audit may be DISABLED for this tenant ($disabledHits indicators found) ***`r`n"
            }

            # Populate grids
            $HarSearchGrid.ItemsSource = @($auditEntries)
            $HarRequestsGrid.ItemsSource = @($allRequests)
            $HarSummaryBox.Text = $summary
            $HarTabControl.Visibility = "Visible"

            $HarFileLabel.Text = "$(Split-Path $dialog.FileName -Leaf) — $($auditEntries.Count) searches, $($allRequests.Count) audit requests"
            $StatusBox.Text = "HAR analysis complete: $($auditEntries.Count) search jobs found."
        }
        catch {
            [System.Windows.MessageBox]::Show("Failed to parse HAR file: $($_.Exception.Message)", "Error")
            $HarFileLabel.Text = "Error loading file"
            $StatusBox.Text = "HAR analysis failed: $($_.Exception.Message)"
        }
    }
})
